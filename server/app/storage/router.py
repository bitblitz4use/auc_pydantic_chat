...
from app.storage.schema import RenameRequest, TagsRequest
...
async def upload_file(
    ...
    """
    logger.info("📤 Upload request for: %s", object_path)
    ...
    try:
        ...
        if tags:
            try:
                ...
                logger.warning("⚠️ Invalid tags JSON: %s", tags)
        ...
        logger.info("✅ Successfully uploaded %s (%s bytes)", object_path, file_size)
        return {
            "status": "success",
        ...
        logger.error("❌ MinIO error uploading %s: %s", object_path, error)
        raise HTTPException(status_code=500, detail=f"Storage error: {str(error)}")
    except Exception as error:
        logger.error("❌ Error uploading %s: %s", object_path, error)
...
async def update_file(
    ...
    Returns:
        Success message with object path
    """
    logger.info("🔄 Update request for: %s", object_path)
    ...
    try:
        ...
        logger.info("✅ Successfully updated %s (%s bytes)", object_path, file_size)
        return {
            "status": "success",
        ...
        logger.error("❌ MinIO error updating %s: %s", object_path, error)
        raise HTTPException(status_code=500, detail=f"Storage error: {str(error)}")
    except Exception as error:
        logger.error("❌ Error updating %s: %s", object_path, error)
        raise HTTPException(status_code=500, detail=f"Update failed: {str(error)}")
...
async def update_file_tags(object_path: str, request: TagsRequest = Body(...)):
    ...
    """
    logger.info("🏷️ Update tags request for: %s", object_path)
    ...
    try:
        ...
        existing_metadata = {
            k: v for k, v in existing_metadata.items() if not k.startswith("x-amz-meta-")
        }
        ...
        logger.info("✅ Successfully updated tags for %s", object_path)
        return {"status": "success", "message": "Tags updated successfully", "tags": request.tags}
    except S3Error as error:
        logger.error("❌ MinIO error updating tags: %s", error)
        raise HTTPException(status_code=500, detail=f"Storage error: {str(error)}")
    except Exception as error:
        logger.error("❌ Error updating tags: %s", error)
        raise HTTPException(status_code=500, detail=f"Update tags failed: {str(error)}")
...
async def rename_file(object_path: str, request: RenameRequest):
    ...
    logger.info("🔄 Rename request: %s -> %s", object_path, new_path)
    ...
    try:
        ...
        copy_source = CopySource(bucket_name, object_path)
        client.copy_object(bucket_name, new_path, copy_source)
        logger.info("✅ Copied %s to %s", object_path, new_path)

        # Delete original object - ensure this happens
        try:
            client.remove_object(bucket_name, object_path)
            logger.info("✅ Deleted original %s", object_path)
        except Exception as delete_error:
            # If deletion fails, try to clean up the copied file
            logger.error("❌ Failed to delete original %s: %s", object_path, delete_error)
            try:
                client.remove_object(bucket_name, new_path)
                logger.warning(
                    "🧹 Cleaned up copied file %s due to deletion failure", new_path
                )
            except Exception as cleanup_error:
                logger.error("❌ Failed to cleanup %s: %s", new_path, cleanup_error)
            raise HTTPException(
                status_code=500,
                detail=f"Rename failed: could not delete original file. {str(delete_error)}",
            )

        logger.info("✅ Successfully renamed %s to %s", object_path, new_path)
        return {
            "status": "success",
        ...
        logger.error("❌ MinIO error renaming %s: %s", object_path, error)
        raise HTTPException(status_code=500, detail=f"Storage error: {str(error)}")
    except Exception as error:
        logger.error("❌ Error renaming %s: %s", object_path, error)
        raise HTTPException(status_code=500, detail=f"Rename failed: {str(error)}")
...
async def delete_file(object_path: str):
    ...
    Returns:
        Success message
    """
    logger.info("🗑️ Delete request for: %s", object_path)
    ...
    try:
        ...
        logger.info("✅ Successfully deleted %s", object_path)
        return {
            "status": "success",
        ...
        logger.error("❌ MinIO error deleting %s: %s", object_path, error)
        raise HTTPException(status_code=500, detail=f"Storage error: {str(error)}")
    except Exception as error:
        logger.error("❌ Error deleting %s: %s", object_path, error)
...
async def list_all_files(prefix: Optional[str] = None, recursive: bool = True):
    ...
    """
    logger.info(
        "📋 List request with prefix: %s, recursive: %s", prefix or "root", recursive
    )
    ...
    try:
        ...
        logger.error("❌ MinIO error listing objects: %s", error)
        raise HTTPException(status_code=500, detail=f"Storage error: {str(error)}")
    except Exception as error:
        logger.error("❌ Error listing objects: %s", error)
        raise HTTPException(status_code=500, detail=f"List failed: {str(error)}")
...
async def get_file_content(object_path: str):
    ...
    """
    logger.info("📥 Download request for: %s", object_path)
    ...
    try:
        ...
        logger.info("✅ Successfully downloaded %s (%s bytes)", object_path, len(content))
        return Response(
            content=content,
        ...
        logger.error("❌ MinIO error downloading %s: %s", object_path, error)
        raise HTTPException(status_code=404, detail=f"File not found: {str(error)}")
    except Exception as error:
        logger.error("❌ Error downloading %s: %s", object_path, error)
...
async def list_files_in_path(path: str, recursive: bool = True, include_tags: bool = False):
    ...
    """
    logger.info(
        "📋 List request for path: %s, recursive: %s, include_tags: %s",
        path,
        recursive,
        include_tags,
    )
    ...
    try:
        ...
        logger.info("✅ Found %s objects in path: %s", len(objects), path)
        return {
            "status": "success",
            "path": path,
            "prefix": prefix,
        ...
        logger.error("❌ MinIO error listing objects in path '%s': %s", path, error)
        raise HTTPException(status_code=500, detail=f"Storage error: {str(error)}")
    except Exception as error:
        logger.error("❌ Error listing objects in path '%s': %s", path, error)
        raise HTTPException(status_code=500, detail=f"List failed: {str(error)}")

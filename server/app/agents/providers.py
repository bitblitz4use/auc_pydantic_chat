...
def create_model(provider: str, model_name: str) -> OpenAIChatModel:
    ...
    if provider == "openai":
        ...
        return OpenAIChatModel(
            model_name=model_name, provider=OpenAIProvider(api_key=config.openai_api_key)
        )
    ...
    if provider == "ollama":
        ...
        client = httpx.AsyncClient(event_hooks={"request": [_fix_ollama_messages]})

        return OpenAIChatModel(
            model_name=model_name,
            provider=OllamaProvider(base_url=config.ollama_base_url, http_client=client),
        )

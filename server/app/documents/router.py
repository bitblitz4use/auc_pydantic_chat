...
from app.documents.schema import FolderRequest, RenameRequest
...
async def list_documents():
    ...
    try:
        ...
        logger.info("✅ Found %s documents and %s folders", doc_count, folder_count)
        return {"status": "success", "count": len(items), "documents": items}
    except S3Error as error:
        logger.error("❌ MinIO error listing documents: %s", error)
        raise HTTPException(status_code=500, detail=f"Storage error: {str(error)}")
    except Exception as error:
        logger.error("❌ Error listing documents: %s", error)
...
async def check_document_exists(document_name: str):
    ...
    Returns:
        Existence status
    """
    logger.info("🔍 Check existence: %s", document_name)
    ...
    try:
        ...
        logger.info("✅ Document '%s' exists: %s", document_name, exists)
        return {"exists": exists, "documentName": document_name}
    except S3Error as error:
        logger.error("❌ MinIO error checking document: %s", error)
        raise HTTPException(status_code=500, detail=f"Storage error: {str(error)}")
    except Exception as error:
        logger.error("❌ Error checking document: %s", error)
        raise HTTPException(status_code=500, detail=f"Check failed: {str(error)}")
...
async def rename_document(document_name: str, request: RenameRequest):
    ...
    logger.info("🔄 Rename request: %s -> %s", document_name, new_name)
    ...
    try:
        ...
        try:
            ...
            logger.info("✅ Copied document: %s -> %s", old_doc_key, new_doc_key)

            # Delete original
            client.remove_object(bucket_name, old_doc_key)
            logger.info("✅ Deleted original document: %s", old_doc_key)
            renamed_count += 1
        ...
        try:
            ...
            logger.info("✅ Copied folder marker: %s -> %s", old_folder_key, new_folder_key)

            # Delete original marker
            client.remove_object(bucket_name, old_folder_key)
            logger.info("✅ Deleted original folder marker: %s", old_folder_key)
            renamed_count += 1
        ...
        for obj in objects_to_move:
            ...
            renamed_count += 1
            logger.info("✅ Moved folder content: %s -> %s", obj.object_name, new_object_name)

        if renamed_count == 0:
            raise HTTPException(status_code=404, detail="Document or folder not found")

        logger.info(
            "✅ Successfully renamed %s item(s): %s -> %s",
            renamed_count,
            document_name,
            new_name,
        )
        ...
        logger.error("❌ MinIO error renaming: %s", error)
        raise HTTPException(status_code=500, detail=f"Storage error: {str(error)}")
    except Exception as error:
        logger.error("❌ Error renaming: %s", error)
...
async def delete_document(document_name: str):
    ...
    Returns:
        Success message
    """
    logger.info("🗑️ Delete request for: %s", document_name)
    ...
    try:
        ...
        try:
            ...
            deleted_count += 1
            logger.info("✅ Deleted folder marker: %s", document_name)
        except S3Error as error:
            if error.code != "NoSuchKey":
                raise
        ...
        for obj in objects_to_delete:
            client.remove_object(bucket_name, obj.object_name)
            deleted_count += 1
            logger.info("✅ Deleted folder content: %s", obj.object_name)
        ...
        item_type = (
            "folder and contents"
            if len(deleted_items) > 1 or deleted_count > 1
            else deleted_items[0]
            if deleted_items
            else "items"
        )
        logger.info("✅ Successfully deleted %s item(s): %s", deleted_count, document_name)
        return {
            "status": "success",
        ...
        logger.error("❌ MinIO error deleting: %s", error)
        raise HTTPException(status_code=500, detail=f"Storage error: {str(error)}")
    except Exception as error:
        logger.error("❌ Error deleting: %s", error)
...
async def create_folder(request: FolderRequest):
    ...
    logger.info("📁 Create folder: %s", folder_path)
    ...
    try:
        ...
        logger.info("✅ Successfully created folder: %s", folder_path)
        return {
            "status": "success",
        ...
        logger.error("❌ MinIO error creating folder: %s", error)
        raise HTTPException(status_code=500, detail=f"Storage error: {str(error)}")
    except Exception as error:
        logger.error("❌ Error creating folder: %s", error)
        raise HTTPException(status_code=500, detail=f"Create folder failed: {str(error)}")

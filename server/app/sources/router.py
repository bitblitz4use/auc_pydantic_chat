...
async def send_progress(source_id: str, stage: str, progress: int, message: str, **extra):
    ...
    progress_data = {"stage": stage, "progress": progress, "message": message, **extra}
    ...
    if source_id in active_connections:
        try:
            ...
            logger.info("📡 %s: %s (%s%%) - %s", source_id[:8], stage, progress, message)
        except Exception as error:
            logger.debug("Failed to send progress for %s: %s", source_id[:8], error)
            active_connections.pop(source_id, None)
    else:
        logger.debug(
            "⏳ %s: Progress buffered (WS not connected yet): %s %s%%",
            source_id[:8],
            stage,
            progress,
        )
...
async def convert_document_background(
    ...
    try:
        ...
        try:
            ...
            logger.warning("⚠️ Could not encode filename for metadata: %s", error)
            metadata["x-amz-meta-original-filename"] = original_filename.encode(
                "ascii", "ignore"
            ).decode("ascii")
        ...
        logger.info("✅ Conversion complete: %s", source_id)

    except Exception as error:
        logger.error("❌ Conversion failed for %s: %s", source_id, error)
        await send_progress(source_id, "error", 0, str(error), error=True)
    finally:
...
async def upload_source(
    ...
    try:
        ...
        logger.info("✅ Stored original: %s", original_path)
...
async def conversion_progress_websocket(websocket: WebSocket, source_id: str):
    ...
    logger.info("🔌 WebSocket connected: %s", source_id[:8])

    try:
        # Send any buffered progress immediately upon connection
        if source_id in conversion_progress:
            logger.info(
                "📤 Sending buffered progress for %s: %s %s%%",
                source_id[:8],
                conversion_progress[source_id].get("stage"),
                conversion_progress[source_id].get("progress"),
            )
        ...
        logger.info("🔌 WebSocket disconnected: %s", source_id[:8])
    finally:
        active_connections.pop(source_id, None)
...
async def list_sources(recursive: bool = True, include_tags: bool = False):
    ...
    """
    logger.info(
        "📋 List sources request: recursive=%s, include_tags=%s", recursive, include_tags
    )
    ...
    try:
                    ...
                    try:
                        ...
                        if encoded_filename:
                            ...
                            try:
                                ...
                                source_map[source_id]["original_filename"] = base64.b64decode(
            ...
            for source_info in source_map.values():
                ...
                if tag_path:
                    try:
                        ...
                        logger.debug("Could not get tags for %s: %s", tag_path, error)
                        source_info["tags"] = []
        ...
        logger.error("❌ MinIO error listing sources: %s", error)
        raise HTTPException(status_code=500, detail=f"Storage error: {str(error)}")
    except Exception as error:
        logger.error("❌ Error listing sources: %s", error)
...
async def get_source_markdown(source_id: str):
    ...
    """
    logger.info("📥 Get markdown request for source: %s", source_id)
    ...
    try:
        ...
        try:
            client.stat_object(bucket_name, markdown_path)
            ...
            logger.info(
                "✅ Successfully retrieved markdown for %s (%s bytes)",
                source_id,
                len(content),
            )
            return Response(
                ...
                headers={"Content-Disposition": 'inline; filename="converted.md"'},
            )
        except S3Error as error:
            if error.code == "NoSuchKey":
                logger.warning("⚠️ Markdown not found for source: %s", source_id)
                raise HTTPException(
                    status_code=404, detail=f"Markdown not found for source: {source_id}"
                )
            raise
        ...
        logger.error("❌ MinIO error getting markdown: %s", error)
        raise HTTPException(status_code=500, detail=f"Storage error: {str(error)}")
    except Exception as error:
        logger.error("❌ Error getting markdown: %s", error)
        raise HTTPException(status_code=500, detail=f"Get markdown failed: {str(error)}")
...
async def delete_source(source_id: str):
    ...
    Returns:
        Success message
    """
    logger.info("🗑️ Delete source request: %s", source_id)
    ...
    try:
        ...
        if not objects:
            logger.warning("⚠️ No files found for source: %s", source_id)
        ...
        for obj in objects:
            try:
                ...
                logger.debug("✅ Deleted: %s", obj["name"])
            except S3Error as error:
                errors.append(f"{obj['name']}: {str(error)}")
                logger.error("❌ Failed to delete %s: %s", obj["name"], error)

        if errors:
            logger.warning("⚠️ Some files failed to delete: %s", errors)
        ...
        logger.info(
            "✅ Successfully deleted source: %s (%s files)", source_id, len(deleted_files)
        )
        return {
            "status": "success",
        ...
        logger.error("❌ MinIO error deleting source: %s", error)
        raise HTTPException(status_code=500, detail=f"Storage error: {str(error)}")
    except Exception as error:
        logger.error("❌ Error deleting source: %s", error)

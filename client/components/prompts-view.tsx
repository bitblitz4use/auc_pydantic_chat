...
import { Wand2, Upload, FileText, Trash2 } from "lucide-react";
...
export function PromptsView() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await fetchPrompts(); // Refresh list to update file metadata
        ...
        <div className="flex h-full w-full items-center justify-center rounded-lg border-2 border-dashed border-border">
          <div className="max-w-md space-y-4 text-center px-8">
            ...
            <p className="text-sm text-muted-foreground">
              No prompts found. Upload your first prompt file to get started.
            </p>
            <div className="pt-2">
              <input
                ref={fileInputRef}
    ...
    <>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="min-w-[800px] h-[600px] max-w-6xl flex flex-col">
          <DialogHeader>
            <DialogTitle>
              {selectedFile?.name.replace("prompts/", "") || "File Content"}
            </DialogTitle>
            <DialogDescription>
